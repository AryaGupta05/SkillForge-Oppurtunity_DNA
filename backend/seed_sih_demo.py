...
def seed_sih_demo_dataset():
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        canonical_emails = [
            "aarav.sharma.demo@example.com",
            "priya.patel.demo@opportunity-dna.in"
        ]

        non_canonical_cands = db.query(models.Candidate).filter(~models.Candidate.email.in_(canonical_emails)).all()
        if non_canonical_cands:
            for c_old in non_canonical_cands:
                ...
                db.delete(c_old)
            db.commit()
            print(f"[*] Successfully purged {len(non_canonical_cands)} non-canonical / legacy test candidate(s).")
        ...
        print(f"[*] 5 Differentiated PMIS Corporate Internships verified in database.")

        # ---------------------------------------------------------------------
        # 3. SEED CONTROLLED DEMO STUDENT (REALISTIC AI/ML PROFILE)
        # ---------------------------------------------------------------------
        demo_email = "aarav.sharma.demo@example.com"
        ...
        readiness = get_readiness_simulation(candidate_id=demo_candidate.id, opportunity_id=top_opp.id, db=db)
        roadmap = get_upskilling_roadmap(candidate_id=demo_candidate.id, opportunity_id=top_opp.id, db=db)
        readiness_2 = get_readiness_simulation(candidate_id=demo_candidate_2.id, opportunity_id=top_opp_2.id, db=db)
